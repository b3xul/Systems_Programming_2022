...
void myList::insertNode(const myClass &val, const int && pos) {
  if (pos == 0) {
    push_front(val);
  } else if (pos == size)
    push_back(val);
...
int myList::findNodeByVal(const char &val) {
  int pos = 0;
  for (node *x = head; x != nullptr; x = x->next, pos++) {
    if (x->value == val) {
      current_el = x;
      return pos;
    }
...
void myList::printList() {
  cout << "head: " << (head ? to_string(head->value) : "") << "\ttail: " << (tail ? to_string(tail->value) : "")
       << "\tcurrent_el: " << (current_el ? to_string(current_el->value) : "") << '\n';
