...
// we don't have to type std::cout to output information, but
...
#define USING_MAIN_1 0
#define USING_MAIN_2 0
#define USING_MAIN_3 0
#define USING_MAIN_4 0
#define USING_MAIN_5 0
#define USING_MAIN_6 0
#define USING_MAIN_7 0
#define USING_MAIN_8 0
#define USING_MAIN_9 0
#define USING_MAIN_10 0
#define USING_MAIN_COMP 1
...
int main() {
  ...
  string xxx;
  cin >> aaa >> bbb >> xxx;
  // This kind of code always works, assuming that there is at least one space or newline between each element in the input
  ...
  printf("This sentence is redirected to a file."); // redirects the output that would normally go to the standard output to a file called output.txt
  int aSmall = 123456789;
  long long BIG = (long long) aSmall * aSmall;
  cout << bbb << "\n"; // -1757895751
  //  (long long) is essential, otherways multiplying 2 int will give an int as result!
  ...
  //In practice, the floating point numbers can be compared as follows (ε = 10−9):
  if (abs(aaa - bbb) < 1e-9) {
    // a and b are equal
  }
  // FINE COMPETITIVE
...
int main(int argc, char **argv) {
  ...
  cout << "cbrt(1000) = " << cbrt(1000) << endl; //cube root of a number
  ...
  int age2 = 43;
  AssignAge(age2);
  cout << "New Age : " << age2 << endl; // age2=43. not 24
...
int main(int argc, char **argv) {
  ...
  /*  function<int(int)> Fib;
    Fib = [&Fib](int n) { return n < 2 ? n : Fib(n - 1) + Fib(n - 2); };
...
// functions that accept 1 parameter
template<typename T>
void Times2(T val) {
  cout << val << " * 2 = " <<
       val * 2 << endl;
...
int main() {
  ...
  nums.push_back(5);
  for (int x: nums)
    cout << x << endl;  // 0 1 2 3 4 5

  // You can access index values, but they are costly
  // because values aren't stored contigously.
  // use multiple arrays instead
  cout << nums[0] << endl;
  ...
  copy(nums4.begin(), nums4.end(),
      ...
      //inserts elements into nums3 at the position pointed to by itr5
       inserter(nums3, itr5));
...
int main() {
  ...
  unique_ptr<int[]> pNums2(new int[amtToStore]);
...
int main() {
  ...
  // Cycle through the deque
  for (int i : deq1)
    cout << i << endl; //11 3 12 6 7 8 9 10
  ...
  // Substitute all values present with the ones passed using assign
...
int main() {
  ...
  // Get iterator to certain value after
  ...
  // Get iterator to value with key >= 1
  auto bart = map1.lower_bound(1);
  cout << "LB : " << bart->second << endl;

  // Get next iterator with key > 1
